    ...
    <div style={styles.container}>
      ...
      <div style={styles.content}>
        <div style={styles.statsCard}>
          ...
          <div style={styles.statValue}>{myTrx.length} <span style={styles.statSub}>pesanan</span></div>
        </div>

        <div style={styles.quickGrid}>
          <button style={{...styles.quickBtn, background: '#1D9E75'}} onClick={() => navigate('/pesan')}>
            <div style={{color: '#fff'}}><IconClipboardList size={32} /></div>
            <div style={styles.quickLabel}>Buat Pesanan Baru</div>
          </button>
          
          <button style={{...styles.quickBtn, background: '#E07B3A'}} onClick={() => navigate('/riwayat')}>
            <div style={{color: '#fff'}}><IconHistory size={32} /></div>
...
const styles = {
  ...
  header: { background: '#1D9E75', padding: '18px 20px 36px', position: 'relative', borderRadius: '0 0 24px 24px' },
  ...
  avatar: { position: 'absolute', top: '16px', right: '18px', width: '40px', height: '40px', borderRadius: '50%', background: '#a0eed3', border: '2.5px solid #fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' },
  content: { padding: '24px 16px 80px', flex: 1, overflowY: 'auto' },
  statsCard: { background: '#fff', borderRadius: '18px', padding: '16px', marginBottom: '24px', border: '1.5px solid #EAE5DA', textAlign: 'center' },
  ...
  quickBtn: { borderRadius: '18px', padding: '20px', display: 'flex', alignItems: 'center', gap: '16px', border: 'none', cursor: 'pointer', textAlign: 'left' },
