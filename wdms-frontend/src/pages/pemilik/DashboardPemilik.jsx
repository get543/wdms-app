...
import { IconToolsKitchen2, IconChartBar, IconPackage, IconCashBanknote } from '@tabler/icons-react';
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <div style={styles.headerDate}>{new Date().toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
      ...
      <div style={styles.content}>
        <div style={styles.adviceCard}>
          ...
          <div>
            ...
            <div style={styles.adviceBody}>Pantau stok bahan baku dan pastikan menu selalu tersedia untuk pelanggan!</div>
          </div>
        </div>
        ...
        <div style={styles.statsRow}>
          <div style={{...styles.statCard, ...styles.incomeCard}}>
            ...
            <div style={{...styles.statValue, color: '#27500A'}}>{formatIDR(stats.total_pendapatan)}</div>
        ...
        <div style={styles.sectionTitle}>Menu Terlaris Hari Ini</div>
        <div style={styles.menuPopular}>
          {menuTerlaris.length === 0
            ? <div style={{color: '#888780', fontSize: '12px', textAlign: 'center'}}>Belum ada data penjualan hari ini.</div>
            : menuTerlaris.slice(0, 3).map((menu, i) => (
              <div key={i} style={styles.menuItem}>
                ...
                <div style={styles.menuBarBg}>
                  <div style={{...styles.menuBarFill, width: `${(menu.total_terjual / maxTerjual * 100)}%`, background: barColors[i]}} />
                </div>
        ...
        <div style={styles.sectionTitle}>Menu Cepat</div>
        <div style={styles.quickGrid}>
          <QuickBtn icon={<IconToolsKitchen2 />} label="Kelola Menu" bg="#C94040" onClick={() => navigate('/menu')} />
          <QuickBtn icon={<IconChartBar />} label="Lihat Laporan" bg="#3B7A57" onClick={() => navigate('/laporan')} />
          <QuickBtn icon={<IconPackage />} label="Kelola Stok" bg="#E07B3A" onClick={() => navigate('/stok')} />
          <QuickBtn icon={<IconCashBanknote />} label="Pengaturan" bg="#5F5E5A" onClick={() => navigate('/pengaturan')} />
        </div>
      </div>
    </div>
  );
...
const styles = {
  ...
  header: { background: '#C94040', padding: '18px 20px 36px', position: 'relative', borderRadius: '0 0 24px 24px' },
  headerTitle: { fontSize: '13px', fontWeight: '700', color: '#fff', opacity: 0.85, textAlign: 'center', marginBottom: '4px' },
  ...
  avatar: { position: 'absolute', top: '16px', right: '18px', width: '40px', height: '40px', borderRadius: '50%', background: '#f5c4c4', border: '2.5px solid #fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px' },
  content: { padding: '24px 16px 80px', flex: 1, overflowY: 'auto' },
  adviceCard: { background: '#FFF8EC', borderRadius: '18px', padding: '13px 16px', marginBottom: '18px', display: 'flex', alignItems: 'flex-start', gap: '10px' },
  ...
  menuPopular: { background: '#fff', borderRadius: '18px', padding: '14px 16px', marginBottom: '18px' },
  ...
  quickBtn: { borderRadius: '18px', padding: '16px 14px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '6px', border: 'none', cursor: 'pointer', textAlign: 'left' },
